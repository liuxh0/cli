	...
	Describe("GetRouteByNameAndSpace", func() {
		...
		When("the route does not have a host", func() {
			It("returns the route and warnings", func() {
				route, warnings, executeErr := actor.GetRouteByNameAndSpace("hostname.domain-name", "space-guid")
		...
		When("the route has a host defined", func() {
			...
			It("returns the route and warnings", func() {
				route, warnings, executeErr := actor.GetRouteByNameAndSpace("hostname.domain-name/the-path", "space-guid")
				Expect(route).To(Equal(Route{
					GUID:       "route1-guid",
					SpaceGUID:  "space-guid",
					DomainGUID: "domain-guid",
		...
		When("invalid domain cannot be found", func() {
			...
			It("returns the error and any warnings", func() {
				_, warnings, executeErr := actor.GetRouteByNameAndSpace("unsplittabledomain/the-path", "space-guid")
		...
		When("getting domain fails", func() {
			...
			It("returns the error and any warnings", func() {
				_, warnings, executeErr := actor.GetRouteByNameAndSpace("hostname.domain-name/the-path", "space-guid")
				Expect(warnings).To(ConsistOf("get-domains-warning"))
				Expect(executeErr).To(Equal(err))
			})
		})
